...
- [ ] リトライ/バックオフ戦略（429/5xx対応、指数バックオフ）
...
- [ ] 型アサーションの安全化（`ok` チェック追加でパニック防止）
- [ ] 設定値の検証（負の値チェック、範囲チェック）

### P4-5: キャッシュ信頼性
- [ ] キャッシュ永続化の改善（以下から選択）
  - (A) sidecar json でメタデータ保存
  - (B) 復元可能なファイル名形式
  - (C) 起動時に復元不可ファイルを削除
- [ ] metacache の最大サイズ制限追加
...
- metacache に最大サイズ制限がない（大量のファイルでメモリ消費増加の可能性）
- 型アサーションの安全性（一部でパニックの可能性あり）
- キャッシュ再起動時の容量不整合
  - `totalSize` と `entries` が乖離し、`evictIfNeeded()` の容量計算が不正確
  - 古いファイルが残ると「cache full」誤判定
  - 改善案：(A) sidecar json で永続化 (B) 復元可能なファイル名 (C) 起動時に復元不可ファイルを削除
