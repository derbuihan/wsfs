...
- [ ] `AllowOther` をデフォルト無効化（`--allow-other` フラグで有効化可能に）
- [ ] キャッシュファイルのパーミッションを `0600` に修正
- [ ] キャッシュディレクトリのパーミッションを `0700` に修正
- [ ] パストラバーサル対策（`path.Clean()` + 親ディレクトリチェック）

### P4-4: コード品質改善
- [ ] タイポ修正（"Faild" → "Failed" in main.go）
- [ ] エラーハンドリングの一貫性向上
...
### 既知のバグ/改善点
- `main.go` に "Faild" タイポあり（"Failed" が正しい）
