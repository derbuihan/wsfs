...
### P4-3: セキュリティ強化
- [x] `Access()` の実装改善（UID/GIDベースのアクセス制御）
  - `--allow-other` 有効時にマウントユーザー以外のアクセスを自動拒否
  - `WSNode` に `ownerUid` と `restrictAccess` フィールドを追加
  - 子ノード作成時に設定を継承（Lookup, Create, Mkdir）
...
### P5-1: ドキュメント整備
- [x] README に本番運用の注意点を追加
  - [x] シングルユーザー環境での使用推奨を明記
  - [x] AllowOther の危険性を記載
  - [x] キャッシュディレクトリの権限について記載
- [x] 推奨される使用シナリオを記載（ローカル開発、CI/CD、Databricksノートブック編集）
- [x] 非推奨の使用シナリオを記載（共有サーバー、機密データ環境）
...
### セキュリティ関連
- ~~`Access()` は現在楽観的許可（常に成功を返す）→ UID/GIDチェック未実装~~ → P4-3 で対応済み
  - `--allow-other` 有効時は自動的にマウントユーザーの UID 以外を拒否
  - `Access()` で `fuse.FromContext(ctx)` から caller の UID を取得してチェック
